...
class DpSeperateOptimizer(DpOptimizer):
    """
    ...
    def run(
        ...
        logger.info("[DpSeperateOptimizer] Running two-stage DP optimization pass...")
        self._logical_opt()
        self._physical_opt()
        ...
        try:
            ...
            logger.info(
                "[DpSeperateOptimizer] Optimized plan cost = %s",
                optimized_cost,
            )
        except Exception as e:
            logger.info(
                "[DpSeperateOptimizer] Failed to calculate optimized plan cost: %s",
                e,
            )

        self._log_optimized_pipeline(tag="DpSeperateOptimizer")
    ...
    def _dp_reorder_offload_cache_fusion(
        ...
        logger.info(
            "[DpSeperateOptimizer] Stage-1 reorder order: %s",
            reordered_inner_ops,
        )
    ...
    def _dp_reorder_only(self, inner_ops: List[int]) -> List[int]:
        ...
        logger.info(
            "[DpSeperateOptimizer] Stage-1 reorder cost (inner ops only): %s",
            dp[final_mask],
        )
        return order_rev
    ...
    def _dp_fixed_order_offload_cache_fusion(
        ...
        logger.info(
            "[DpSeperateOptimizer] Stage-2 fixed-order DP cost (inner ops only): %s",
            dp[n][final_state],
        )
