...
OUTPUT_ROOT="outputs/motivation_multimodal/operator_scaling"
while [[ $# -gt 0 ]]; do
  case "$1" in
...
if [[ -f "$PID_FILE" ]] && kill -0 "$(<"$PID_FILE")" 2>/dev/null; then
  echo "Scaling benchmark is already running with PID $(<"$PID_FILE")" >&2
  exit 1
fi
...
python -m evaluation.motivation_multimodal.scaling \
