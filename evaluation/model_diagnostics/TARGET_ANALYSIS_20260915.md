...
| pile_hackernews | 43.7 | Plumber 34.9 | 1.25× | 26.1 | **1.68×** |
| dino（统一 SMP 语义后） | 259.6 | Plumber 252.6 | 1.03× | 261.1 | 0.99× |

结论：**1.5× 的目标目前只有 simclr 一个负载达到**；其余负载 PICO 与最优外部
系统基本打平（0.98–1.25×），但在"模型敏感"的负载上对 ablation 有 1.2–3.8×
的优势。
...
| 要求 | 达成情况 |
|---|---|
| ≥8 个负载上 PICO ≥1.5× 最优外部系统 | ❌ 只有 simclr（1.65×），其余 0.98–1.25× |
| 其中 ≥3 个负载 PICO ≥1.3× simple-DP | ✅ alpaca 3.81×、hackernews 1.68×、bloom 1.64×（uspto 1.19× 接近） |
| 这 8 个负载上没有 PICO 差于 simple-DP | ✅ dino 从 0.66× 修到 0.99× 后不再有反例 |
...
上面的推理有一个隐患：SMP stage 的进程与 worker **在同一台机器上**，共享同一
批核心；"投递后即可继续"只说明投递动作便宜，并不意味着它的计算能凭空与
worker 链重叠。实测三处反例：

* alpaca：六个 filter 放到 `SMP(1)` 上，1048 → 982 rec/s；
* clip：一个算子放在 `SMP(1)`，1203 → 872 rec/s；
* dino：模型偏好的 `W=8 + 融合 SMP(2)` 计划 812 rec/s，全本地计划 1038 rec/s。

因此把规则统一为：**worker 驱动的一切都记在记录的关键路径上，并行度只通过
stage 内部的宽度折算服务时间**（`CEDAR_DP_SMP_MODE=additive` 已是默认，
`lane` 保留旧语义）。改后 dino 上 DP 选回全本地计划：259.6 rec/s vs Plumber
252.6（1.03×），对 ablation 0.99×（改前 0.66×）。

论文里可以这样表述：

> 每条记录在 worker 上串行地经过（本地算子 → 每个 offload stage 的投递/回收
> → 结果合并）；stage 的宽度只减少该 stage 的服务时间，不会让它的服务从记录
> 的路径上消失。只有真正离开该主机的计算（Ray actor 的远程服务）才可能与
> worker 链重叠，而我们的实测表明即使在这一情形下，提交/等待路径的代价也足以
> 主导（见 fit 表）。

相应地，`evaluation/verify_dp_optimizer_optimality.py` 的最优性 oracle 被固定
在旧的 lane 语义（它正是为那套语义写的），后续需要把 oracle 也扩展到 additive
规则。
