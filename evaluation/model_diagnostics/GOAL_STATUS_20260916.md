...
|---|---|---|---|---|---|---|
| coco | 275.0 | Plumber 35.3 | **7.80×** | 253.0 | 1.09× | ✓ |
| simclr（已标定 W） | **483.4** | Plumber 250.0 | **1.93×** | 430.9 | 1.12× | ✓ |
| simclrv2（已标定 W） | **454.9** | Plumber 248.8 | **1.83×** | 410.5 | 1.11× | ✓ |
| simclrv2_views4 | 425.0 | Plumber 258.8 | **1.64×** | 406.9 | 1.04× | ✓ |
...
### 2. 逐负载 worker 竞争标定把 PICO 自身提高了 12–21%（本轮已实施）
...
| coco | 272.1 (W=32) | 272.1 (W=32) | 7.80× | 1.09× |

注意一个**协议陷阱**：这批曲线是在"去掉每 worker 1 核运行时预留"
（`CEDAR_DP_RUNTIME_CPU_RESERVE_PER_WORKER=0`）下测的，这个开关同时放宽了
stage 宽度预算，因此小 W 的点被高估；simclrv2_views4 就因此从 W=32（425.0）
被改判到 W=16（415.1，反而更慢）。正式使用前应当在**正式协议**下重测曲线。

### 3. Ray 边界比 SMP 贵 4.5 倍/字节（新增，llava 上标定）
| 后端 | 固定延迟 | 吞吐 |
|---|---|---|
...
### 4. 含 GPU 算子的融合 Ray stage 被高估（新发现，llava 反面案例）
...
### 5. 多视图 recipe 的计划空间确实有 1.6–2.0× 余地
...
2. **把"算子成本随实际输入规模变化"真正用起来**：simclrv2 一族现在的顺序收益
   完全来自联合搜索撞对了顺序；若把每个算子的 per-byte 成本按重排后的真实
   输入规模计价（Crop 提前能把后续像素算子的输入降到 1/2.5），有望再拿 20–30%，
   这才是把 PICO/simple-DP 从 1.11× 推到 1.3× 的最直接路径。
