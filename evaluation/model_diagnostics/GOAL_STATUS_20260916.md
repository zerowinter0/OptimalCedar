...
### 回退的根因（2026-09-16 15:50，已修复）

**根因：affine 固定项只在一条定价路径上生效。** 我加 affine 时把 `+ fixed[i]`
加进了 `_BlockCostIndex`（精确搜索用的定价），但漏了 `candidate_for_order`
（**构造 incumbent** 用的定价）。后果是一条完整的因果链：

1. incumbent 被系统性低估（14.05/16.45），而搜索真正能造出的最优块要 26.99 起；
2. DP 把 incumbent 的价格当作**分支定界剪枝上界**（任何超过它的前缀标签直接丢弃）；
3. 上界低于所有可达标签 ⇒ 前沿在第 7 层被清空 ⇒ `Extensible DP failed: no
   feasible final state` ⇒ 触发我加的回退。

**修复（三条，均已提交）**：

| 修复 | 内容 |
|---|---|
| 定价一致 | `candidate_for_order` 现在用与 `_BlockCostIndex` **完全相同**的递推（含 affine 固定项）。incumbent 与搜索的价差从 2.5× 降到 <0.1% |
| 上界可达性校验 | 贪心整块估计只有在该块**真的被 provider 提供**时才允许进入上界（我们抓到过一个不被搜索空间允许的 7 算子 INPROCESS 整块估计） |
| 下界修正 | suffix 下界原先把"每算子原始 per-byte 之和"当下界，但目标函数会按实测宽度曲线打折——这是拿上界当下界；现在按最宽候选折扣 |
...
| 计划 | W | 模型给的分（per-record） | 模型给的 aggregate（per-record/W） | 实测 |
|---|---|---|---|---|
| PICO：1×Ray(32 actor) + 本地融合链 | 32 | 41.49 | **1.30 ms** | 230.9 rec/s（4.33 ms/record） |
| ablation：本地两条 stage（21 worker + 1×SMP(1)） | 21 | 31.19 | 1.49 ms | 312–465 rec/s（2.2–3.2 ms/record） |

模型一致地偏好 W=32 + Ray 的计划（1.30 < 1.49），实测却相反。逐项拆开：

- profile 的隔离测量说 `_read` 在 Ray 上 8.57 ms、本地冷读 24.87 ms（已用 IO 规则钳到不低于本地）；
- 计划日志显示执行形态是 `ray_stages=1, global_ray_actors=32`（每 worker 1 个远端 actor），
  真实每条记录要过 **3 次跨主机的 500KB–800KB 往返**；identity 边界实测约 **10 ms/次**，
  即至少 30 ms/record 的投递+回收，而模型只按 0.5×in+0.5×out 记一次；
- 远端 actor 还要承担 decode/resample/融合链的服务，模型把 Ray stage 服务按
  `width^0.5` 折算并放在与本地链**相加**的位置，但**没有用远端 64 核的总容量给聚合吞吐封顶**。

**下一步（唯一还缺的一块）**：给 Ray 通道加"远端机器容量上界"——计划用 W 个 worker
各带自己的远端 actor 时，远端每秒要处理 `W × (每记录远端工作量)`，这个量不能超过
远端 CPU 预算；一旦超了，聚合吞吐必须被远端容量卡住。按上面数据推算，这一步能把
W=32+Ray 的 aggregate 从 1.30 ms 提到 ≈2.5–4 ms，DP 就会选本地 SMP 流水线（312–465 rec/s，
即 1.25–1.65× 最优外部系统）。
