import pathlib
from evaluation.pipelines.simclrv2 import cedar_dataset as original_simclrv2
import torch
...
class SimCLRV2Feature(Feature):
    ...
    def _compose(self, source_pipes: List[Pipe]):
        fp = source_pipes[0]
        # Diagnostic variant: the same augmentation set applied in the
        # "shrink first" order (decode, crop, grayscale, jitter, flip, blur,
        # float, normalize).  Used to measure how much of the end-to-end time
        # an augmentation chain's operator order is worth; the recorded
        # SimCLR cells keep the upstream order.
        ...
        fp = MapperPipe(fp, transforms.RandomHorizontalFlip())
...
def get_dataset(spec: CedarEvalSpec) -> DataSet:
    data_dir = (
        pathlib.Path(original_simclrv2.__file__).resolve().parents[2].joinpath(DATASET_LOC)
    )
