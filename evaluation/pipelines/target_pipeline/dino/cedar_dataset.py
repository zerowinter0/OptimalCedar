"""DINO multi-crop preprocessing with explicit Cedar operators."""
...
class DINOFeature(Feature):
    def __init__(self, batch_size=1, image_root="", views=10):
        super().__init__()
        self.batch_size = batch_size
        self.image_root = image_root
        # Multi-crop workloads repeat the same per-view operator chain. The
        # view count is parameterized so the exact joint DP (2^n subset
        # metadata) can be evaluated on a reduced but structurally identical
        # pipeline.
    ...
    def _compose(self, source_pipes):
        ...
        for view in range(self.views):
            ...
            fp = MapperPipe(fp, OnField(field, GaussianBlur(
                1.0 if view == 0 else 0.1 if view == 1 else 0.5)), tag=f"{field}_blur")
            pil_ops = [f"{field}_{op}" for op in
                       ("crop", "flip", "jitter", "gray", "blur")]
            if view == 1:
                fp = MapperPipe(fp, OnField(field, Solarize(0.2)),
                                tag=f"{field}_solarize")
                pil_ops.append(f"{field}_solarize")
