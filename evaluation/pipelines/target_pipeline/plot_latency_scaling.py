"""Backend/modality-separated multi-operator per-record latency plots."""
import argparse
import html
import json
import re
...
def render(source, output):
    ...
    if not np.isfinite(raw["ms_per_record"]).all() or not (raw["ms_per_record"] > 0).all():
        raise ValueError("Invalid per-record times")
    expected_ms = 1000 * raw["seconds"] / (raw["calls"] * raw["records_per_call"])
    if not np.allclose(raw["ms_per_record"], expected_ms, rtol=1e-10, atol=1e-12):
        raise ValueError("Per-record times do not match recorded timings")
    data = raw
    summary = data.groupby(["operator_index", "backend", "point"]).agg(
        input_bytes=("input_bytes", "mean"),
        mean_ms_per_record=("ms_per_record", "mean"),
    ...
    data.to_csv(output / "latency_raw.csv", index=False)
    summary.to_csv(output / "latency_summary.csv", index=False)
    ...
    for backend in ("local", "smp", "ray"):
        for modality in names:
            ...
            entries = []
            with PdfPages(folder / "latency.pdf") as pdf:
                for group, indices in groups.items():
                    ...
                    for start in range(0, len(indices), 9):
                        ...
                        for color, index in zip(plt.get_cmap("tab10").colors, chunk):
                            ...
                            y = values.mean_ms_per_record.to_numpy()
                        ...
                        ax.set_ylabel("Processing time per record (ms)")
                        ...
                        fig.text(.1, .015,
                                 "Mean of 3 rounds; worker compute only; controlled input sizes.",
            ...
            (folder / "index.html").write_text(
                ...
                '<a href="latency.pdf">Download PDF</a></p><h1>' + title + '</h1>' + body)
    ...
    for backend in ("local", "smp", "ray"):
        cells = "".join(
            ...
            f'<a href="{backend}_{mod}/latency.pdf">PDF</a></td>' for mod in names)
        rows.append(f"<tr><th>{backend.upper()}</th>{cells}</tr>")
    (output / "index.html").write_text(
        '<meta charset="utf-8"><title>Operator processing time per record</title>'
        ...
        '纵轴为每条数据处理时间（毫秒/条），直接使用原始ms_per_record，'
        '对3轮取算术均值，不归一化、不显示波动范围。横轴为实际字节量，线性刻度。</p>'
        '<table><tr><th>后端</th><th>图片</th><th>文本</th><th>图文混合</th></tr>'
        + "".join(rows) + '</table>'
        '<p>图文混合组为CLIP/BLIP的收集与批处理算子；图片读取按文件字节量，'
        '其他图片算子按解码后的实际处理字段计量。曲线反映worker内部计算，'
        '不包含通信。Batcher按实际条数折算为每条耗时。这些数据来自受控尺寸实验，部分输入大小并非原流水线可达。</p>')
    (output / "catalog.json").write_text(json.dumps(catalog, indent=2))
    (output / "README.md").write_text(
        "# Per-record processing time plots\n\n"
        "Y: arithmetic mean of three ms_per_record measurements. No normalization, "
        "error bars, or uncertainty bands. X: actual active input size, linear scale.\n\n"
        "Timings are checked against 1000 * seconds / (calls * records_per_call). "
        "Batcher times are divided by the number of records processed. "
        "No measurements were rerun. All 161 operator occurrences are preserved.\n\n"
        "Worker compute only; IPC, startup and input construction excluded. "
        "Inputs are from the original controlled size sweep, including counterfactual "
        "sizes not reachable in the original pipeline.\n")
