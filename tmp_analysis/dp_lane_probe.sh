#!/bin/bash
# Compare the two Ray accounting modes on one workload by planning and running
# the PICO optimizer alone.
#   dp_lane_probe.sh <workload> <samples> <mode> <outdir>
set -u
REPO=/home/xieruiyang/OptimalCedar
cd "$REPO"
DEV=optimalcedar-torch201-dev
RAY=172.23.166.105:6379
W=$1; N=$2; MODE=$3; OUT=${4:-/tmp/screen_lane}
mkdir -p "$OUT"
...
docker exec -e CEDAR_RAY_PLACEMENT_RESOURCE=cedar_remote \
  ...
  python -u outputs/plumber_bench_20260912/entry.py evaluation/compare_optimizer_perf.py \
    ...
    --results_path /workspace/OptimalCedar/outputs/lane_probe/${W}_${MODE}.json" \
  > "$OUT/${W}_${MODE}.log" 2>&1
echo "[lane_probe] $W mode=$MODE exit=$?"
